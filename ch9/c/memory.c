...
/*
两件事要注意。
（1）pte_ptr和pde_ptr这两个函数返回的是能够访问到vaddr所在pte及pde的
新虚拟地址new_vaddr，new_vaddr经过处理器处理32位地址的三个步骤，
最终指向vaddr的pte及pde所在的物理地址。
因此，这两个函数的功能等同于：给我一个新的虚拟地址new_vaddr，
让它指向vaddr所在的pde及pte，
也就是让new_vaddr指向pde及pte所在的物理地址。
（2）这两个函数中的参数vaddr，
可以是已经分配、在页表中存在的，
也可以是尚未分配，目前页表中不存在的虚拟地址，
pte_ptr和pde_ptr这两个函数只是根据虚拟地址转换的规则计算出
vaddr对应的pte及pde的虚拟地址，与vaddr所在的pte及pde是否存在无关。
*/


/*函数pte_ptr，它接受一个参数vaddr，
    功能是得到地址vaddr所在pte的指针，
    强调下，指针的值也就是虚拟地址，
    故此函数实际返回的是能够访问vaddr所在pte的虚拟地址。 
*/
